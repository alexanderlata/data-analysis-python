...
В курсе используется генератор `default_rng`: он создаётся один раз, все случайные числа берутся из него методами. Зерно делает результат воспроизводимым: `default_rng(42)` даёт одни и те же числа при каждом запуске, `default_rng()` без зерна — каждый раз новые.

- `rng = np.random.default_rng(seed)` - создание генератора; `seed` - целое зерно или ничего
- `rng.integers(low, high, size)` - целые числа от `low` до `high`, правая граница не включается
- `rng.random(size)` - дробные числа от 0 до 1
- `rng.choice(a, size, replace=True)` - случайный выбор элементов из массива или списка; `replace=False` - без повторов
- `rng.permutation(a)` - перемешанная копия массива; `rng.shuffle(a)` - перемешивание на месте
- `rng.normal(loc, scale, size)` - нормальное распределение со средним `loc` и стандартным отклонением `scale`
- `rng.uniform(low, high, size)` - равномерное распределение от `low` до `high`